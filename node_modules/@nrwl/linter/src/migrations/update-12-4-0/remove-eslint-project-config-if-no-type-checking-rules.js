...
function updateProjectESLintConfigs(host) {
    ...
    projects.forEach((p) => {
        const eslintConfigPath = path_1.join(p.root, '.eslintrc.json');
        if (!host.exists(eslintConfigPath)) {
            return;
        }
        return devkit_1.updateJson(host, eslintConfigPath, rules_requiring_type_checking_1.removeParserOptionsProjectIfNotRequired);
    });
}
function removeESLintProjectConfigIfNoTypeCheckingRules(host) {
    return tslib_1.__awaiter(this, void 0, void 0, function* () {
        ...
        const rootESLintConfig = devkit_1.readJson(host, '.eslintrc.json');
        if (rules_requiring_type_checking_1.hasRulesRequiringTypeChecking(rootESLintConfig)) {
            return;
        }
