...
function findMainRenderStatement(source) {
    ...
    const calls = find_nodes_1.findNodes(source, ts.SyntaxKind.CallExpression);
    ...
    const imports = find_nodes_1.findNodes(source, ts.SyntaxKind.ImportDeclaration);
...
function findDefaultExportDeclaration(source) {
    ...
    if (identifier) {
        const variables = find_nodes_1.findNodes(source, ts.SyntaxKind.VariableDeclaration);
        const fns = find_nodes_1.findNodes(source, ts.SyntaxKind.FunctionDeclaration);
        const cls = find_nodes_1.findNodes(source, ts.SyntaxKind.ClassDeclaration);
...
function findDefaultExportIdentifier(source) {
    const exports = find_nodes_1.findNodes(source, ts.SyntaxKind.ExportAssignment);
...
function findElements(source, tagName) {
    const nodes = find_nodes_1.findNodes(source, [
...
function addInitialRoutes(sourcePath, source) {
    const jsxClosingElements = find_nodes_1.findNodes(source, [
...
function addReduxStoreToMain(sourcePath, source) {
    ...
    if (!renderStmt) {
        devkit_1.logger.warn(`Could not find ReactDOM.render in ${sourcePath}`);
        return [];
    }
    const jsx = renderStmt.arguments[0];
    return [
        ...addImport(source, `import { configureStore, getDefaultMiddleware } from '@reduxjs/toolkit';
...
function getComponentName(sourceFile) {
    ...
    if (!(defaultExport &&
        find_nodes_1.findNodes(defaultExport, ts.SyntaxKind.JsxElement).length > 0)) {
        return null;
    }
...
function getComponentPropsInterface(sourceFile) {
    ...
    if (propsTypeName) {
        return find_nodes_1.findNodes(sourceFile, ts.SyntaxKind.InterfaceDeclaration).find((x) => {
